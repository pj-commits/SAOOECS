...
<div x-data="{ listOfEventsModal: false, modal: true }" class="w-auto h-auto lg:w-3/4">
    ...
    <div x-data="calendar()" x-init="[initDate(), getNoOfDays()]" x-cloak>
      <div class="container mx-auto" @load.window="addEvent({{ $forms }})">
      ...
      <x-modal name="listOfEventsModal">
        <template x-for="event in listOfEvents">	
          <div
            ...
            :class="{
              'border-green-200 text-semantic-success bg-green-100': event.form_type === 'APF',
              'border-blue-200 text-semantic-info bg-blue-100': event.form_type === 'BRF',
              'border-yellow-200 text-primary-yellow bg-yellow-100': event.form_type === 'NR',
              'border-rose-200 text-semantic-warning bg-rose-100': event.form_type === 'LF',
            }"
          >
